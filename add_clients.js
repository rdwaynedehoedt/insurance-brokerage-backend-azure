      ...
      try {
        const query = `
          INSERT INTO clients (
            ...
            sales_rep_id, policies, created_at, updated_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
        `;
        ...
        const params = [
          ...
          client.sales_rep_id, client.policies
        ];
        
        const [result] = await connection.execute(query, params);
        insertedCount++;
        console.log(`Inserted client: ${client.client_name}`);
      } catch (err) {
        console.error(`Error inserting client ${client.client_name}:`, err.message);
      }
    }
    
    console.log(`Successfully inserted ${insertedCount} clients`);
